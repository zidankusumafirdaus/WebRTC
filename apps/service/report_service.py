...
def _report_bucket() -> str:
    return getattr(Config, 'SUPABASE_REPORTS_BUCKET', None) or getattr(Config, 'SUPABASE_ATTACHMENTS_BUCKET', 'attachments')
...
def create_report(counselor_id: int, conversation_id: int, title: Optional[str], content: Optional[str]) -> Dict[str, Any]:
...
def attach_report_file(report_id: int, counselor_id: int, file, filename: str) -> Dict[str, Any]:
    supabase = get_supabase_client()
    report_resp = supabase.table('reports').select('report_id, counselor_id').eq('report_id', report_id).single().execute()
...
def list_reports_for_counselor_user(counselor_id: int, user_id: int) -> List[Dict[str, Any]]:
    supabase = get_supabase_client()
    resp = supabase.table('reports').select('*').eq('user_id', user_id).eq('counselor_id', counselor_id).order('created_at', desc=True).execute()
    return resp.data or []


def get_report_for_user(report_id: int, user_id: int) -> Optional[Dict[str, Any]]:
    supabase = get_supabase_client()
    resp = supabase.table('reports').select('*').eq('report_id', report_id).eq('user_id', user_id).single().execute()
    return resp.data if resp else None


def get_report_attachment(report_id: int) -> Optional[Dict[str, Any]]:
    supabase = get_supabase_client()
    resp = supabase.table('report_attachments').select('filename, file_path, size, uploaded_at').eq('report_id', report_id).single().execute()
    return resp.data if resp else None
