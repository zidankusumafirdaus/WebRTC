...
def verify_credentials(username: str, password: str):
    ...
    if not user_data:
        # Try counselors
        resp = supabase.table('counselors').select('*').eq('username', username).execute()
        user_data = resp.data
        table = 'counselors'
    if not user_data:
        return None
...
def is_username_available(username: str) -> bool:
    supabase = get_supabase_client()
    resp_u = supabase.table('users').select('user_id').eq('username', username).execute()
    resp_c = supabase.table('counselors').select('counselor_id').eq('username', username).execute()
...
def create_counselor(username: str, hashed_password: str, display_name: str, bio=None, qualifications=None) -> int:
    supabase = get_supabase_client()
    new_counselor = {
        'username': username,
        'password': hashed_password,
