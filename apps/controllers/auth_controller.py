...
def register_user_controller():
    ...
    if not username or not password or not display_name:
        return jsonify({'error': 'Username, password, dan display_name wajib diisi.'}), 400
    ...
    try:
        ...
        user_id = create_user(username=username, hashed_password=hashed, display_name=display_name)
...
def register_counselor_controller():
    ...
    try:
        ...
        payload = {'message': 'Counselor berhasil didaftarkan.', 'user_id': str(counselor_id)}
        return jsonify(RegisterResponseSchema().dump(payload)), 201
    except Exception as e:
        return jsonify({'error': str(e)}), 500
