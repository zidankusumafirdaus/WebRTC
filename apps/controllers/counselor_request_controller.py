...
def create_counselor_request():
    ...
    try:
        request_id = create_request(user_id=user_id, counselor_id=counselor_id, message=message, expires_at=datetime.utcnow() + timedelta(hours=24))
        payload = {'message': 'Request konsultasi berhasil dikirim.', 'request_id': request_id}
...
def get_pending_requests():
    ...
    try:
        items = list_pending_requests(counselor_id=counselor_id, now_iso=datetime.utcnow().isoformat())
...
def respond_to_request():
    ...
    if 'response_message' in data or 'message' in data:
        return jsonify({'error': 'response_message tidak diperbolehkan untuk endpoint ini.'}), 400
    try:
        validated = RespondRequestSchema().load(data)
    except ValidationError as e:
        return jsonify({'error': e.messages}), 400
    try:
        conversation_id = respond_request(counselor_id=counselor_id, request_id=int(validated['request_id']), status=validated['status'])
        return jsonify({'message': f"Request {validated['status']}.", 'conversation_id': conversation_id}), 200
    except Exception as e:
        return jsonify({'error': str(e)}), 500
...
def get_user_reports(user_id: int):
    counselor_id = int(get_jwt_identity())
    try:
        reports = get_reports_if_accepted(counselor_id=counselor_id, user_id=int(user_id))
        return jsonify(ReportSchema(many=True).dump(reports))
    except PermissionError:
        return jsonify({'error': 'Tidak ada request yang diterima untuk melihat report.'}), 403
    except Exception as e:
        return jsonify({'error': str(e)}), 500